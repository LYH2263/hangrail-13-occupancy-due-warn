...
def seed_if_empty(db: Session) -> None:
    ...
    orders = [
        ...
        WorkOrder(store_id=store.id, ticket_code="HR-2006", garment_name="真丝衬衫", length_cm=30, status="hung", due_at=now + timedelta(hours=6), hung_at=now - timedelta(hours=2)),
    ]
    db.add_all(orders)
    db.flush()
    db.add_all(
        [
            ...
            RailPlacement(rail_id=r1.id, order_id=orders[5].id, start_cm=80, end_cm=110),
