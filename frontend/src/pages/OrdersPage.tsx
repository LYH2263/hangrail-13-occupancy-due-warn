...
import { DueLevel, dueBadgeClass, dueLabel, formatDue } from "../utils/due";
type O = { id: number; ticket_code: string; garment_name: string; length_cm: number; status: string; due_at: string; due_level: DueLevel };
export default function OrdersPage() {
  ...
  return (<>
    ...
    <table className="table"><thead><tr><th>票号</th><th>衣物</th><th>衣长</th><th>状态</th><th>到期</th><th>预警</th><th></th></tr></thead>
    <tbody>{rows.map(o => <tr key={o.id}><td className="mono">{o.ticket_code}</td><td>{o.garment_name}</td><td className="mono">{o.length_cm}cm</td><td>{o.status}</td>
      <td className="mono">{formatDue(o.due_at)}</td>
      <td><i className={dueBadgeClass(o.due_level)}>{dueLabel(o.due_level)}</i></td>
