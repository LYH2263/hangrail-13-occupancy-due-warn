...
export default function OrdersPage() {
  ...
  return (<>
    ...
    <tbody>{rows.map(o => {
      const lvl = dueLevel(o.due_at);
      return <tr key={o.id}><td className="mono">{o.ticket_code}</td><td>{o.garment_name}</td><td className="mono">{o.length_cm}cm</td><td>{o.status}</td>
        <td className={`mono due-td due-td--${lvl}`}>
          <span className="due-td-time">{formatDue(o.due_at)}</span>
          <span className="due-td-hint">{dueHint(o.due_at)}</span>
        </td>
